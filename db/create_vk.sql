...
CREATE TABLE users (
 ...
 lastname VARCHAR(50),  -- В PostgreSQL комментарии к колонкам добавляются отдельно
 email VARCHAR(120) UNIQUE
);
-- Добавление комментария к колонке lastname
...
CREATE TABLE messages (
 id SERIAL PRIMARY KEY,
 from_user_id INTEGER NOT NULL REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
 to_user_id INTEGER NOT NULL REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
 body TEXT,
 created_at TIMESTAMP DEFAULT NOW()
);
INSERT INTO messages (from_user_id, to_user_id, body, created_at) VALUES
(1, 2, 'Voluptatem ut quaerat quia. Pariatur esse amet ratione qui quia. In necessitatibus reprehenderit et. Nam accusantium aut qui quae nesciunt non.', NOW() + INTERVAL '1 minute'),
(2, 1, 'Sint dolores et debitis est ducimus. Aut et quia beatae minus. Ipsa rerum totam modi sunt sed. Voluptas atque eum et odio ea molestias ipsam architecto.', NOW() + INTERVAL '3 minute'),
(3, 1, 'Sed mollitia quo sequi nisi est tenetur at rerum. Sed quibusdam illo ea facilis nemo sequi. Et tempora repudiandae saepe quo.', NOW() + INTERVAL '5 minute'),
(4, 1, 'Quod dicta omnis placeat id et officiis et. Beatae enim aut aliquid neque occaecati odit. Facere eum distinctio assumenda omnis est delectus magnam.', NOW() + INTERVAL '11 minute'),
(1, 5, 'Voluptas omnis enim quia porro debitis facilis eaque ut. Id inventore non corrupti doloremque consequuntur. Molestiae molestiae deleniti exercitationem sunt qui ea accusamus deserunt.', NOW() + INTERVAL '12 minute'),
(1, 6, 'Rerum labore culpa et laboriosam eum totam. Quidem pariatur sit alias. Atque doloribus ratione eum rem dolor vitae saepe.', NOW() + INTERVAL '14 minute'),
(1, 7, 'Perspiciatis temporibus doloribus debitis. Et inventore labore eos modi. Quo temporibus corporis minus. Accusamus aspernatur nihil nobis placeat molestiae et commodi eaque.', NOW() + INTERVAL '15 minute'),
(8, 1, 'Suscipit dolore voluptas et sit vero et sint. Rem ut ratione voluptatum assumenda nesciunt ea. Quas qui qui atque ut. Similique et praesentium non voluptate iure. Eum aperiam officia quia dolorem.', NOW() + INTERVAL '21 minute'),
(9, 3, 'Et quia libero aut vitae minus. Rerum a blanditiis debitis sit nam. Veniam quasi aut autem ratione dolorem. Sunt quo similique dolorem odit totam sint sed.', NOW() + INTERVAL '22 minute'),
(10, 2, 'Praesentium molestias quia aut odio. Est quis eius ut animi optio molestiae. Amet tempore sequi blanditiis in est.', NOW() + INTERVAL '25 minute'),
(8, 3, 'Molestiae laudantium quibusdam porro est alias placeat assumenda. Ut consequatur rerum officiis exercitationem eveniet. Qui eum maxime sed in.', NOW() + INTERVAL '27 minute'),
(8, 1, 'Quo asperiores et id veritatis placeat. Aperiam ut sit exercitationem iste vel nisi fugit quia. Suscipit labore error ducimus quaerat distinctio quae quasi.', NOW() + INTERVAL '28 minute'),
(8, 1, 'Earum sunt quia sed harum modi accusamus. Quia dolor laboriosam asperiores aliquam quia. Sint id quasi et cumque qui minima ut quo. Autem sed laudantium officiis sit sit.', NOW() + INTERVAL '33 minute'),
(4, 1, 'Aut enim sint voluptas saepe. Ut tenetur quos rem earum sint inventore fugiat. Eaque recusandae similique earum laborum.', NOW() + INTERVAL '35 minute'),
(4, 1, 'Nisi rerum officiis officiis aut ad voluptates autem. Dolor nesciunt eum qui eos dignissimos culpa iste. Atque qui vitae quos odit inventore eum. Quam et voluptas quia amet.', NOW() + INTERVAL '35 minute'),
(4, 1, 'Consequatur ut et repellat non voluptatem nihil veritatis. Vel deleniti omnis et consequuntur. Et doloribus reprehenderit sed earum quas velit labore.', NOW() + INTERVAL '37 minute'),
(2, 1, 'Iste deserunt in et et. Corrupti rerum a veritatis harum. Ratione consequatur est ut deserunt dolores.', NOW() + INTERVAL '37 minute'),
(8, 1, 'Dicta non inventore autem incidunt accusamus amet distinctio. Aut laborum nam ab maxime. Maxime minima blanditiis et neque. Et laboriosam qui at deserunt magnam.', NOW() + INTERVAL '41 minute'),
(8, 1, 'Amet ad dolorum distinctio excepturi possimus quia. Adipisci veniam porro ipsum ipsum tempora est blanditiis. Magni ut quia eius qui.', NOW() + INTERVAL '42 minute'),
(8, 1, 'Porro aperiam voluptate quo eos nobis. Qui blanditiis cum id eos. Est sit reprehenderit consequatur eum corporis. Molestias quia quo sit architecto aut.', NOW() + INTERVAL '50 minute');
-- заявки на дружбу
...
-- Создание индекса отдельно (PostgreSQL не поддерживает инлайн-синтаксис INDEX в CREATE TABLE)
...
CREATE TABLE users_communities (
 ...
 community_id INTEGER NOT NULL REFERENCES communities(id) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE media (
 id SERIAL PRIMARY KEY,
 user_id INTEGER NOT NULL REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
 media_type_id INTEGER REFERENCES media_types(id) ON UPDATE CASCADE ON DELETE SET NULL,
 body TEXT,
...
CREATE TABLE likes (
 id SERIAL PRIMARY KEY,
 user_id INTEGER NOT NULL REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
 media_id INTEGER NOT NULL REFERENCES media(id) ON UPDATE CASCADE ON DELETE CASCADE
);
INSERT INTO likes (user_id, media_id) VALUES
(1, 1), (2, 1), (3, 1), (4, 1), (5, 2), (6, 2), (7, 2), (8, 8), (1, 9), (10, 15),
(7, 11), (5, 12), (6, 13), (1, 14), (1, 15), (1, 16), (1, 17), (1, 18), (1, 19), (1, 20);
...
CREATE TABLE profiles (
 ...
 gender CHAR(1),
 birthday DATE,
 photo_id INTEGER REFERENCES media(id) ON UPDATE CASCADE ON DELETE SET NULL,
